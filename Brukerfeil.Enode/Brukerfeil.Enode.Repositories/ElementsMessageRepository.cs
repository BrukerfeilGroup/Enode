    ...
    public class ElementsMessageRepository : IElementsMessageRepository
    {
        ...
        public async Task<IEnumerable<ElementsMessage>> GetElementsMessagesAsync(string organizationId, Direction direction)
        {
            try
            {
                ...
                var request = new HttpRequestMessage(HttpMethod.Get, $"{orgConfig.ElementsNcore}/SenderRecipient?$filter=CreatedDate le {currentDate} and ConversationId ne null and IsRecipient eq {recipientValue} &$expand=SendingMethod,SendingStatus,RegistryEntry($expand=Case)&$top=50&$orderby=CreatedDate desc&Database={orgConfig.Database}&ExternalSystemName={orgConfig.ExternalSystemName}");
        ...
        public async Task<IEnumerable<ElementsMessage>> GetElementsMessagesBySenderIdAsync(string orgId, string senderId)
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Get, $"{orgConfig.ElementsNcore}/SenderRecipient?$filter=ExternalId eq '{senderId}' and ConversationId ne null &$expand=SendingMethod,SendingStatus&Database={orgConfig.Database}&ExternalSystemName={orgConfig.ExternalSystemName}");
            var response = await Client.SendAsync(request);
            using HttpContent content = response.Content;
        ...
        public async Task<IEnumerable<ElementsMessage>> GetElementsMessagesByReceiverIdAsync(string orgId, string receiverId)
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Get, $"{orgConfig.ElementsNcore}/SenderRecipient?$filter=ExternalId eq '{receiverId}' and ConversationId ne null &$expand=SendingMethod,SendingStatus&Database={orgConfig.Database}&ExternalSystemName={orgConfig.ExternalSystemName}");
            var response = await Client.SendAsync(request);
            using HttpContent content = response.Content;
